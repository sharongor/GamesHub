...
public class ServiceGamesForCompany {
        ...
        try {
            while (i <= 3) {
                ...
                for(JsonElement je:results){
                    ...
                    JsonElement storesElement = obj.get("stores");

                    if (storesElement.isJsonArray()) {
                        JsonArray storesArray = storesElement.getAsJsonArray();

                        for (JsonElement storeElement : storesArray) {
                            JsonObject storeObject = storeElement.getAsJsonObject();
                            JsonObject store = storeObject.get("store").getAsJsonObject();
